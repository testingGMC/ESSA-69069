...
export default class Graph extends React.Component {
   ...
   * @returns {undefined}
   */
  _onDragEnd = () => !this.state.config.staticGraph && this.state.config.automaticRearrangeAfterDropNode && this.state.simulation.alphaTarget(D3_CONST.SIMULATION_ALPHA_TARGET).restart()
   ...
   * @returns {undefined}
   */
  _setNodeHighlightedValue = (id, value = false) => this._tick(graphHelper.updateNodeHighlightedValue(this.state.nodes, this.state.links, this.state.config, id, value))
  ...
  componentWillReceiveProps (nextProps) {
    ...
    const configUpdated = !utils.isObjectEmpty(nextProps.config) && !utils.isDeepEqual(nextProps.config, this.state.config)
  ...
  render () {
    ...
    // console.log(Object.keys(this.state.links).length)
      ...
      <div id={`${this.state.id}-${CONST.GRAPH_WRAPPER_ID}`}>
        <svg style={svgStyle}>
          <g id={`${this.state.id}-${CONST.GRAPH_CONTAINER_ID}`}>
            <defs>
              <marker id='arrow' markerWidth='10' markerHeight='10' refX='12px' refY='3' orient='auto' markerUnits='strokeWidth' overflow='hidden'>
