  ...
  populateFromText(text)
      {
          ...
          let line = 0
          let col = 0
          let version = new Version(0)
          this.struct = [[]]
          this.controller.vector = new VersionVector(this.siteId)
          this.vector = this.controller.vector
          
          let characterArray = Array.from(text)
          ...
          if (c === '\n')
            ...
            version.counter++
            this.vector.update(version)
            let position = [];
            position.push(new Identifier(i, 0));
            var character = new Char(""+c, version.counter, 0, position);
            this.insertChar(character, {line: line, ch: col});
            i++;
            col++;
          })
        }
